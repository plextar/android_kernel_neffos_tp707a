...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
	...
	[GPIO_SWCH_HIGH] = {"aud_swch_high", false, NULL},
	[GPIO_SWCH_LOW] = {"aud_swch_low", false, NULL},
	[GPIO_RCV_LOW] = {"rcv_low", false, NULL},
...
int AudDrv_GPIO_EXTAMP_Select(int bEnable, int mode)
{
	...
	if (bEnable == 1) {
		if ((mode == 8) || (mode == 3)){
			retval = pinctrl_select_state(pinctrlaud,aud_gpios[GPIO_SWCH_HIGH].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_SWCH_LOW] pins\n");
		}else if(mode == 9){
			retval = pinctrl_select_state(pinctrlaud,aud_gpios[GPIO_SWCH_LOW].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_SWCH_HIGH] pins\n");
		}

		retval = pinctrl_select_state(pinctrlaud,aud_gpios[GPIO_RCV_LOW].gpioctrl);
			if (retval)
			pr_err("could not set aud_gpios[GPIO_RCV_LOW] pins\n");
