...
// [zhangdong start] add exhibition mode
		...
		const char *buf, size_t size)
{
	...
	unsigned int val = 0;
	if (buf != NULL && size != 0) {
		chr_err("[store_exhibition_mode] buf is %s and size is %zu\n", buf, size);
...
static DEVICE_ATTR(exhibition_mode, 0664, NULL, store_exhibition_mode);        /* 664 */
		...
		char *buf)
{
	...
	ret_value = g_total_charger_on_time * CHARGING_INTERVAL;
	chr_err("[show_total_charger_on_time] g_total_charger_on_time: %d\n", ret_value);
	...
	while (1) {
		...
		if (is_disable_charger() == false) {
			if (is_charger_on == true) {
				...
				// [zhangdong] Add for exhibition mode support
				g_total_charger_on_time++;
			} else {
				g_total_charger_on_time = 0;
			}
		} else
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	// [zhangdong start] add exhibition mode support
	ret = device_create_file(&(pdev->dev), &dev_attr_exhibition_mode);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_total_charger_on_time);
	if (ret)
		goto _out;
	// [zhangdong end]
