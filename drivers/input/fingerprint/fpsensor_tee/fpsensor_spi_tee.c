...
static char version[]="leadcore ver 1.0";
static char vendor_name[50]="leadcore";
...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	enable_irq_wake(fpsensor_dev->irq);
        if (retval == 0) {
            fpsensor_debug(ERR_LOG, " irq thread request success!\n");
        } else {
...
/*        if (s_DEVINFO_fpsensor != NULL) {
    ...
    FUNC_EXIT();
    fpsensor_debug(ERR_LOG, "retval ccccccc(%d)\n", retval);
    return retval;
}
...
//-add by hzb for ontim debug
    ...
    /*rsee get sensor id*/    
    rsee_client_get_fpid(&fp_id);
    fpsensor_debug(DEBUG_LOG, "fp_sensor_id = %x\n", fp_id);
    if(fp_id != 0x7332)
    {
        fpsensor_debug(ERR_LOG, "fp_sensor_id mis_match,exit!\n");
        goto err2;
    }
    /*rsee get sensor id*/
