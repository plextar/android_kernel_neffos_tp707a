...
/* [liguanxiong] Add for fingerprint unlock acceleration */
...
static irqreturn_t cf_irq_handler(int irq, void * dev_id)
{
	...
	if (allow_speed_up) {
		input_report_key(cf_dev->input, KEY_FP_SPEED_UP, 1);
		input_sync(cf_dev->input);
		input_report_key(cf_dev->input, KEY_FP_SPEED_UP, 0);
		input_sync(cf_dev->input);
...
static int cf_input_init(struct cf_device *cf_dev)
{
	...
	/* [liguanxiong start] Add oem keyevent */
	cf_dev->input->name = "fp_oem_kpd";
	...
	__set_bit(KEY_FP_SPEED_UP, cf_dev->input->keybit);
	/* [liguanxiong end] Add oem keyevent */
...
static int cf_report_key_event(struct input_dev* input, cf_key_event_t* kevent)
{
	...
	/* [liguanxiong start] Modify keyevent map */
	switch (kevent->key) {
		case CF_KEY_HOME:	  key_code = KEY_HOME;   break;
		case CF_KEY_MENU:	  key_code = KEY_MENU;   break;
		case CF_KEY_BACK:	  key_code = KEY_BACK;   break;
		case CF_KEY_DOWNUP:	key_code = KEY_FP_TOUCH;    break;
		case CF_KEY_ONETAP: key_code = KEY_FP_CLICK;    break;
		case CF_KEY_DOUBLETAP: key_code = KEY_FP_DOUBLE_CLICK;    break;
		case CF_KEY_LONGTOUCH: key_code = KEY_FP_LONG_CLICK;    break;
		case CF_KEY_ENTER:	key_code = KEY_ENTER;  break;
		case CF_KEY_UP: 	  key_code = KEY_FP_RIGHT;	   break;
		case CF_KEY_LEFT:	  key_code = KEY_FP_DOWN;   break;
		case CF_KEY_RIGHT:	key_code = KEY_FP_UP;  break;
		case CF_KEY_DOWN:	  key_code = KEY_FP_LEFT;   break;
		case CF_KEY_WAKEUP: key_code = KEY_WAKEUP; break;

		default: break;
	}
	cf_debug(DEBUG_LOG, "cf_report_key_event: kevent->key = %d, kevent->value = %d\n", kevent->key, kevent->value);
	/* [liguanxiong end] */

	/* [liguanxiong start] immediately release KEY_FP_DOUBLE_CLICK keyevent */
	if (key_code == KEY_FP_LONG_CLICK && kevent->value != 2) {
		if (kevent->value == 1) {
...
/* [liguanxiong start] */
#define CHIPS_LIB_VERSION    "unknow"
#define CHIPS_DRIVER_VERSION "unknow"
#define CHIPS_CHIP_TYPE      "unknow"
#define CHIPS_VENDOR      "ChipSailing"

static ssize_t chips_fp_info_show(struct device *dev,
...
static ssize_t chips_fp_info_store(struct device *dev,
...
static DEVICE_ATTR(fp_info, S_IRUGO | S_IWUSR | S_IWGRP, chips_fp_info_show, chips_fp_info_store);
/* [liguanxiong end] */
