...
/* [lichenggang start] modify for gesture */
extern atomic_t double_enable;
extern int tp_suspend;
...
* Return: 
***********************************************************************/
static ssize_t fts_gesture_id_ontim_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    int ret = 0;
    char page[512];
    /* [liliwen] Add double tap support id */
    ret = sprintf(page, "0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
                    0x01, 0x02,0x04, 0x05, 0x06, 0x07,0x0C, 0x0D);
...
static ssize_t fts_gestrue_ontim_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int ret = 0;
    int state = 0;
    char buf[10] = {0};
...
* Return: read char number
***********************************************************************/
static ssize_t fts_gestrue_ontim_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    int len = 0;
    int ret = 0;
    char page[512];
...
extern ssize_t fts_gesture_coordinate_open(struct file *file, char __user *user_buf, size_t count, loff_t *ppos);
static const struct file_operations fts_gesture_coordinate_fops = {
    .read = fts_gesture_coordinate_open,
    .open = simple_open,
    .owner = THIS_MODULE,
};
#endif
...
static ssize_t fts_tp_info_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    int len = 0;
    int ret = 0;
...
//    }
//
//    5yyif (copy_from_user(&writebuf, buff, count)) {
//        FTS_ERROR("[APK]: copy from user error!!");
//        return -EFAULT;
//    }
//
...
int fts_create_proc(void)
{
    ...
    refresh_senstive_ontim();
    fts_file_ontim = proc_create("tp_thresh_value", 0777, fts_dir_ontim, &fts_senstive_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc dist thresh  entry fail");
        return -ENOMEM;
    }

    fts_file_ontim = proc_create("support_gesture_id", 0777, fts_dir_ontim, &fts_gesture_id_ontim_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc dist thresh  entry fail");
        return -ENOMEM;
    }
    ...
    fts_file_ontim = proc_create("gesture_coordinate", 0777, fts_dir_ontim, &fts_gesture_coordinate_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc black gesture entry fail");
        return -ENOMEM;
    }
#endif
    fts_file_ontim = proc_create("tp_info", 0777, fts_dir_ontim, &fts_tp_info_fops);
    if (NULL == fts_file_ontim) {
        FTS_ERROR("create proc fts_tp_info_fops entry fail");
        return -ENOMEM;
    }
