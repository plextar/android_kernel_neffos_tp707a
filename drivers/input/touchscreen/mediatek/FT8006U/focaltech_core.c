...
int tp_suspend = 0;
struct wake_lock fts_gesture_wakelock;/* [lichenggang] solve gesture invalid */
...
static irqreturn_t tpd_eint_interrupt_handler(int irq, void *dev_id)
{
    ...
    wake_lock_timeout(&fts_gesture_wakelock, HZ * 2);/* [lichenggang] solve gesture invalid */
    return IRQ_HANDLED;
}
...
static int tpd_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    /* [lichenggang] solve gesture invalid */
    ret = enable_irq_wake(ts_data->irq);
    if (ret)
        FTS_ERROR("tpd Can't enable IRQ as wake source: %d\n", ret);
    /* [lichenggang end] */
    ft_touch_irq = ts_data->irq;/* [lichenggang]add for gesture */
...
static int touch_event_handler(void *unused)
{
    ...
    #if FTS_GESTURE_EN
    u8 state = 0;/* [lichenggang]add for gesture */
    #endif
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    fts_i2c_client = client;/* [lichenggang]add for gesture */
    ...
    wake_lock_init(&fts_gesture_wakelock, WAKE_LOCK_SUSPEND, "fts_gesture_wakelock");/* [lichenggang] solve gesture invalid */
...
err_input_init:
 wake_lock_destroy(&fts_gesture_wakelock);/* [lichenggang] solve gesture invalid */
#if FTS_POWER_SOURCE_CUST_EN
    fts_power_suspend();
//+ add by fxz
...
static void tpd_suspend(struct device *h)
{
    ...
    FTS_FUNC_ENTER();
    tp_suspend = 1;/* [lichenggang]add for gesture */
...
#if FTS_GESTURE_EN
    if(1 == atomic_read(&double_enable)){/* [lichenggang ]add for gesture */
...
static void tpd_resume(struct device *h)
{
    ...
    FTS_FUNC_ENTER();
    tp_suspend = 0;/* [lichenggang]add for gesture */
...
/* [lichengang start] Add for gesture */
#if FTS_GESTURE_EN
