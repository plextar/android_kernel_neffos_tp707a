...
static int himax_HSEN_show(struct seq_file *m, void *v)
{
    struct himax_ts_data *ts = private_ts;
    size_t ret = 0;

    E("enter::%s, HSEN_enable=%d\n",__func__, ts->HSEN_enable);
    seq_printf(m, "%d\n", ts->HSEN_enable);
    return ret;
}

static int himax_HSEN_open(struct inode *inode, struct file *file) {
    I("%s: enter, %d \n", __func__, __LINE__);
    return single_open(file, himax_HSEN_show, NULL);
}
