            ...
            buf[GEST_PTLG_ID_LEN+1] == GEST_PTLG_HDR_ID2)
    {
        ...
        if(gest_pt_cnt)
        {
            ...
            if(( gesture_flag == 0x51 ) || ( gesture_flag == 0x52 ) || ( gesture_flag == 0x54 )){
                ...
                coordinate_report[12] = 0;//indicate direction
...
static int touch_num_count=0;/* [lichenggang] add log for debug*/
    ...
    if (hx_point_num != 0 )
    {
        ...
        for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++)
        {
            ...
            if(x >= 0 && x <= ts->pdata->abs_x_max && y >= 0 && y <= ts->pdata->abs_y_max)
            {
                ...
                /* [lichenggang start] add irq count*/
                touch_num_count++;
                if(touch_num_count == 10){
                    pr_err("tp707_tpd_down:[%d %d]\n",x,y);
                    touch_num_count = 0;
                }
                /* [lichenggang end] */
#ifndef	HX_PROTOCOL_A
...
#ifdef HX_AUTO_UPDATE_FW
FW_force_upgrade:
    pr_err("panel_fw_ver=0x%x\n",ic_data->vendor_fw_ver);
    pr_err("file_i_FW_VER=0x%x\n",g_i_FW_VER);
    pr_err("panel_config_ver=0x%x\n",ic_data->vendor_config_ver);
    pr_err("file_i_CFG_VER=0x%x\n",g_i_CFG_VER);
