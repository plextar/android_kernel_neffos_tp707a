...
#define PFX "ov5675mipi_Sensor.c"
...
#define RG_TYPICAL   	0x164	//modified when write WB !!!
#define BG_TYPICAL		0x169	//modified when write WB !!!
struct otp_struct {
//	flag bit[7]:[6] = 0:1 group1 有效  bit[5]:[4] = 0:1 group2 有效   bit[3]:[2] = 0:1
	kal_uint8 flag; // 0x7010  		
	kal_uint8 module_integrator_id; // 0x7011 | 0x7019 | 0x7021
	kal_uint8 year_month; 			// 0x7012 | 0x701A
	kal_uint8 day;                  // 0x7013 | 0x701B
	kal_uint8 awb_rg;               // 0x7014 | 0x701C
	kal_uint8 awb_bg;               // 0x7015 | 0x701D
	kal_uint8 golden_awb_rg;        // 0x7016 | 0x701E
	kal_uint8 golden_awb_bg;        // 0x7017 | 0x701F
	kal_uint8 otp_checksum;         // 0x7118 | 0x7020
};

// return 0 成功     return 1 失败
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	addr = 0;
	if(((*otp_ptr).flag & 0xc0) == 0x40) {// group1 有效
		addr = 0x7011; // base address of info group 1
		LOG_INF(" use OTP group1 \n");
	}
	else if(((*otp_ptr).flag & 0x30) == 0x10) {// group2 有效
		addr = 0x7019; // base address of info group 2
		LOG_INF(" use OTP group2 \n");
	}
 	else if(((*otp_ptr).flag & 0xc) == 0x4) {// group3 有效
		addr = 0x7021; // base address of info group 3
		LOG_INF(" use OTP group3 \n");
	}
	else
	{
		LOG_INF(" return 1  OTP group1 group2 group3 all error \n");
		return 1;
	}

	(*otp_ptr).module_integrator_id = read_cmos_sensor(addr);
	(*otp_ptr).year_month = read_cmos_sensor( addr + 1);
	(*otp_ptr).day = read_cmos_sensor( addr + 2);
	(*otp_ptr).awb_rg = read_cmos_sensor( addr + 3);
	(*otp_ptr).awb_bg = read_cmos_sensor(addr + 4);
	(*otp_ptr).golden_awb_rg = read_cmos_sensor( addr + 5);
	(*otp_ptr).golden_awb_bg = read_cmos_sensor(addr + 6);


	checksum = ((*otp_ptr).module_integrator_id + 
                (*otp_ptr).year_month + 
                (*otp_ptr).day + 
                (*otp_ptr).awb_rg +
                (*otp_ptr).awb_bg +
                (*otp_ptr).golden_awb_rg +
                (*otp_ptr).golden_awb_bg)%255 + 1;
	(*otp_ptr).otp_checksum = read_cmos_sensor(addr + 7);
	if((*otp_ptr).otp_checksum == checksum)
	{
		LOG_INF("checksum Successful  year_month=0x%x  day=0x%x\n", (*otp_ptr).year_month, (*otp_ptr).day);
	}
	else
	{
		LOG_INF("(*otp_ptr).module_integrator_id=0x%x \n", (*otp_ptr).module_integrator_id);
		LOG_INF("(*otp_ptr).year_month=0x%x \n", (*otp_ptr).year_month);
		LOG_INF("(*otp_ptr).day=0x%x \n", (*otp_ptr).day);
		LOG_INF("(*otp_ptr).awb_rg=0x%x \n", (*otp_ptr).awb_rg);
		LOG_INF("(*otp_ptr).awb_bg=0x%x \n", (*otp_ptr).awb_bg);
		LOG_INF("(*otp_ptr).golden_awb_rg=0x%x \n", (*otp_ptr).golden_awb_rg);
		LOG_INF("(*otp_ptr).golden_awb_bg=0x%x \n", (*otp_ptr).golden_awb_bg);

		LOG_INF("return 1  checksum failed    (*otp_ptr).otp_checksum=%d   checksum=%d\n", (*otp_ptr).otp_checksum, checksum);
		return 1;
	}			
...
static void apply_otp(struct otp_struct *otp_ptr)
{
	...
	bg = (*otp_ptr).awb_bg;
	//calculate G gain
	R_gain = ( (*otp_ptr).golden_awb_rg * 1000) / rg;
	B_gain = ( (*otp_ptr).golden_awb_bg * 1000) / bg;
	G_gain = 1000;
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
            ...
            {		
				...
				LOG_INF("Read sensor id ok,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}
...
// add by yinzhimeng begin
	...
	{
		LOG_INF("Apply the sensor OTP error \n");
	}
