...
/* [wuzhe start] modify high temp system reset action */
...
void shutdown_func(unsigned long val)
{
	pr_err("[shutdown] force shutdown!\n");
	machine_power_off();
}
EXPORT_SYMBOL(shutdown_func);
static DEFINE_TIMER(sd_timer, shutdown_func, 0, 0);
