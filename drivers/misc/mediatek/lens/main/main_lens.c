...
#define AF_DEBUG
#ifdef AF_DEBUG
#define LOG_INF(format, args...) pr_err("main_lens.c [%s](%d) " format, __func__, __LINE__, ##args)
#else
...
{
	LOG_INF("AFIOC_S_SETPOWERCTRL regulator_put regVCAMAF=%p    Stage=%d\n", regVCAMAF, Stage);
...
//				if (strncmp(CONFIG_ARCH_MTK_PROJECT, "k39tv1_bsp", 17) == 0)
                {
                    LOG_INF(" vldo28 \n");
				...
				LOG_INF("[Init] regulator_get regVCAMAF=%p\n", regVCAMAF);
...
static long AF_SetMotorName(__user struct stAF_MotorName *pstMotorName)
{
	...
	for (i = 0; i < MAX_NUM_OF_LENS; i++) 
    {
		...
		if (strcmp(stMotorName.uMotorName, g_stAF_DrvList[i].uDrvName) == 0) 
        {
            ...
            LOG_INF("return i4RetValue=%ld   find  Motor  %s \n", i4RetValue, stMotorName.uMotorName );           
            return i4RetValue;
		}
	}
	LOG_INF("return i4RetValue=%ld   no find  Motor \n", i4RetValue );
	return i4RetValue;
}
