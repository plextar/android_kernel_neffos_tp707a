...
static void eem_set_eem_volt(struct eem_det *det)
{
	...
	for (i = 0; i < det->num_freq_tbl; i++) {
		...
		case EEM_CTRL_L:
			if ((turbocode == 1) && (i == 0))
				det->volt_tbl_pmic[i] = det->ops->volt_2_pmic(det, CPU_TURBO_OPP0_VOLT);
			else
				det->volt_tbl_pmic[i] = min(
				(unsigned int)(clamp(
					det->ops->eem_2_pmic(det,
		...
		case EEM_CTRL_GPU:
			if ((turbocode == 1) && (i == 0))
				det->volt_tbl_pmic[i] = det->ops->volt_2_pmic(det, GPU_TURBO_OPP0_VOLT);
			else if ((turbocode == 1) && (i == 1))
				det->volt_tbl_pmic[i] = det->ops->volt_2_pmic(det, GPU_TURBO_OPP1_VOLT);
			else
				det->volt_tbl_pmic[i] = min(
				(unsigned int)(clamp(
					det->ops->eem_2_pmic(det,
