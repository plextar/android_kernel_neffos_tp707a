...
static void mtkfb_shutdown1(struct platform_device *pdev)
{
	enum mtkfb_power_mode prev_pm = primary_display_get_power_mode();
	MTKFB_LOG("[FB Driver] mtkfb_shutdown1()\n");
//    printk(KERN_ERR "yzwyzw %s %d\n",__func__,__LINE__);
	...
	if (!lcd_fps)
		...
		msleep(3 * 100000 / lcd_fps);	/* Delay 2 frames. */
	...
	primary_display_suspend();
	debug_print_power_mode_check(prev_pm, FB_SUSPEND);
	MTKFB_LOG("[FB Driver] leave mtkfb_shutdown1\n");
}
