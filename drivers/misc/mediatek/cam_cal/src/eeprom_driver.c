...
#ifdef CAM_CAL_DEBUG
/*#include <linux/log.h>*/
#define PK_INF(format, args...)     pr_debug(PFX " eeprom_driver.c[%s](%d) " format, __func__, __LINE__, ##args)
#define PK_DBG(format, args...)     pr_debug(PFX " eeprom_driver.c[%s](%d) " format, __func__, __LINE__, ##args)
#define PK_ERR(format, args...)     pr_debug(PFX " eeprom_driver.c[%s](%d) " format, __func__, __LINE__, ##args)
#else
...
static int EEPROM_get_cmd_info(unsigned int sensorID, struct stCAM_CAL_CMD_INFO_STRUCT *cmdInfo)
{
	...
	if (pCamCalList != NULL && pCamCalFunc != NULL) 
    {
        ...
        {
			PK_DBG("pCamCalList[%d].sensorID=0x%x   sensorID=0x%x\n", i, pCamCalList[i].sensorID,  sensorID);
...
/* 				if (pCamCalList[i].checkFunc(cmdInfo->client, pCamCalFunc[0].readCamCalData)) 
                ...
                {
					PK_DBG("pCamCalList[%d].checkFunc  NULL \n", i);
                } */
    ...
    {
		PK_DBG("pCamCalList = 0x%p   pCamCalFunc = 0x%p\n", pCamCalList, pCamCalFunc);
    }
	return 0;

}

static struct stCAM_CAL_CMD_INFO_STRUCT *EEPROM_get_cmd_info_ex(unsigned int sensorID, unsigned int deviceID)
{
	int i = 0;

	/* To check device ID */       // 4
		...
		for (i = 0; i < CAM_CAL_I2C_MAX_SENSOR; i++) 
        {
			...
			if (g_camCalDrvInfo[i].sensorID == 0) 
            {
				...
				if (g_camCalDrvInfo[i].readCMDFunc != NULL) 
                {
					...
					PK_DBG("deviceID=%d, SensorID=0x%x\n",deviceID, sensorID);
				}
                else
                {
                    PK_DBG("g_camCalDrvInfo[i].readCMDFunc = NULL \n"); 
                }
				break;
			}
		}
    ...
    {
		PK_DBG(" return &g_camCalDrvInfo[%d] \n", i);
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		if (pcmdInf != NULL) 
        {
			if (pcmdInf->readCMDFunc != NULL)
            {
				...
				PK_DBG("ptempbuf->u4Offset=0x%x  *pu1Params=0x%x   ptempbuf->u4Length=%d  \n",
                       ptempbuf->u4Offset, *pu1Params, ptempbuf->u4Length);
            }
			else 
            {
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params, (u8 *)pu1Params, ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pu1Params);
			PK_DBG("return -EFAULT  ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
