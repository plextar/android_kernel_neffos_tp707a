...
//+ add by fanxzh
#define GN_MTK_BSP_PS_DYNAMIC_CALI // [wuzhe]
//- add by fanxzh
...
struct wake_lock ltr578_wakelock; // [wuzhe]
/*----------------------------------------------------------------------------*/
...
static int ltr578_dynamic_calibrate(void)
{
		...
		// [wuzhe start]
		 if (noise < (dynamic_cali + 200)) {
				...
				if (noise < 19) {
						atomic_set(&obj->ps_thd_val_high,  noise+32);
						atomic_set(&obj->ps_thd_val_low, noise+15);
				}
				else if (noise < 35) {
						atomic_set(&obj->ps_thd_val_high,  noise+33);
						atomic_set(&obj->ps_thd_val_low, noise+16);
				} else {
						atomic_set(&obj->ps_thd_val_high,  noise+45);
						atomic_set(&obj->ps_thd_val_low, noise+23);
				}
...
static irqreturn_t ltr578_eint_handler(int irq, void *desc)
{
		...
		ltr578_eint_func();
		wake_lock_timeout(&ltr578_wakelock, HZ * 2); // [wuzhe]
...
int ltr578_setup_eint(struct i2c_client *client)
{
		...
		if (ltr578_obj->irq_node) {
				...
				/* [wuzhe start] Enable ALS-eint wake up */
				ret = enable_irq_wake(ltr578_obj->irq);
				if (ret)
				{
					APS_ERR("%s: enable_irq_wake(%d) failed, ret=(%d)\n", __func__, ltr578_obj->irq, ret);
				}
				/* [wuzhe end] */
...
err_out:
						...
						const struct i2c_device_id *id)
		{
				...
				wake_lock_init(&ltr578_wakelock, WAKE_LOCK_SUSPEND, "ltr578_wakelock"); // [wuzhe]
...
exit_init_failed:
				...
				/* exit_kfree: */
				kfree(obj);
				wake_lock_destroy(&ltr578_wakelock); // [wuzhe]
exit:
