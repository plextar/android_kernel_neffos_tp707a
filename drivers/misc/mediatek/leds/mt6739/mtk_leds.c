...
#define GREEN_TR1_SEL      PMIC_ISINK_BREATH1_TR1_SEL
#define GREEN_TR2_SEL      PMIC_ISINK_BREATH1_TR2_SEL
#define GREEN_TF1_SEL      PMIC_ISINK_BREATH1_TF1_SEL
#define GREEN_TF2_SEL      PMIC_ISINK_BREATH1_TF2_SEL
#define GREEN_TON_SEL      PMIC_ISINK_BREATH1_TON_SEL
#define GREEN_TOFF_SEL     PMIC_ISINK_BREATH1_TOFF_SEL
// [tanghonghui end]
...
struct pinctrl_state *led_gpio_high = NULL;
struct pinctrl_state *led_gpio_low = NULL;
 int led_gpio_init(void)
{
	int ret=0;

	pinctrled = devm_pinctrl_get(&led_pdev->dev);
	if (IS_ERR(pinctrled)) {
		ret = PTR_ERR(pinctrled);
		LEDS_DEBUG("%s;Cannot find pinctrled!\n",__func__);
	}
	else
	{
		LEDS_DEBUG(" %s;%s;find pinctrled!\n",__func__,led_pdev->name);

		led_gpio_high = pinctrl_lookup_state(pinctrled, "led_gpio_high");
		if (IS_ERR(led_gpio_high)) {
			ret = PTR_ERR(led_gpio_high);
			LEDS_DEBUG("%s;Cannot find led_gpio_high pinctrl state!\n",__func__);
			led_gpio_high = NULL;
		}

		led_gpio_low = pinctrl_lookup_state(pinctrled, "led_gpio_low");
		if (IS_ERR(led_gpio_low)) {
			ret = PTR_ERR(led_gpio_low);
			LEDS_DEBUG("%s;Cannot find led_gpio_low pinctrl state!\n",__func__);
			led_gpio_low = NULL;
		}
	}

	LEDS_DEBUG("%s;exit ret=%x;\n",__func__,ret);
	return ret;
}
...
static void led_gpio_set(int level)
{
	...
	{
		if(led_gpio_high)
			pinctrl_select_state(pinctrled,led_gpio_high);
	}
	else
	{
		if(led_gpio_low)	
			pinctrl_select_state(pinctrled,led_gpio_low);
	}
}
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	case MT65XX_LED_PMIC_NLED_ISINK1:
		...
		pmic_set_register_value(PMIC_ISINK_CH1_STEP, ISINK_0);	/* [tanghonghui modify] ISINK_3 -> ISINK_0 */
		pmic_set_register_value(PMIC_ISINK_CH1_BIAS_EN, NLED_ON);
		pmic_set_register_value(PMIC_ISINK_CHOP1_EN, NLED_ON);
		pmic_set_register_value(PMIC_ISINK_CH1_EN, NLED_ON);

		// [tanghonghui start] For led brightness curve
