...
static int fl_set_level(struct flashlight_dev *fdev, int level)
{
	...
	if (!fdev || !fdev->ops) {
		LOG_INF("Failed with no flashlight ops\n");
		return -EINVAL;
	}
	...
	if (pt_is_low(pt_low_vol, pt_low_bat, pt_over_cur))
    {
		if (fdev->low_pt_level >= 0 && level > fdev->low_pt_level) 
        {
			...
			LOG_INF("Set level to (%d) since pt(%d,%d,%d), pt strict(%d)\n", level, pt_low_vol, pt_low_bat, pt_over_cur, pt_strict);
		}
    }
#endif
	/* ioctl */
...
static int fl_enable(struct flashlight_dev *fdev, int enable)
{
	...
	if (pt_is_low(pt_low_vol, pt_low_bat, pt_over_cur) == 2)
        ...
        {
			enable = 0;
			LOG_INF("Failed to enable since pt(%d,%d,%d), pt strict(%d)\n",
...
int flashlight_verify_type_index(int type_index)
{
	if (type_index < 0 || type_index >= FLASHLIGHT_TYPE_MAX) {
		LOG_INF("type index (%d) is not valid\n", type_index);
		return -1;
	}
	return 0;
}
...
int flashlight_verify_ct_index(int ct_index)
{
	if (ct_index < 0 || ct_index >= FLASHLIGHT_CT_MAX) {
		LOG_INF("ct index (%d) is not valid\n", ct_index);
		return -1;
	}
	return 0;
}
...
{                                       // 2  
	if (part_index < 0 || part_index >= FLASHLIGHT_PART_MAX) {
		LOG_INF("part index (%d) is not valid\n", part_index);
		return -1;
	}
	return 0;
}
...
static int flashlight_verify_arg(struct flashlight_arg fl_arg)
{
	...
	if (fl_arg.level < -1 || fl_arg.level > FLASHLIGHT_ARG_LEVEL_MAX) {
		LOG_INF("level (%d) is not valid\n", fl_arg.level);
		return -1;
	}
	if (fl_arg.dur < 0 || fl_arg.dur > FLASHLIGHT_ARG_DUR_MAX) {
		LOG_INF("duration (%d) is not valid\n", fl_arg.dur);
		return -1;
	}
...
int flashlight_get_ct_id(int ct_index)
{
	if (flashlight_verify_ct_index(ct_index)) {
		LOG_INF("color temperature index (%d) is not valid\n", ct_index);
		return -1;
	}
...
int flashlight_get_type_index(int type_id)
{
	if (type_id < 1 || type_id > FLASHLIGHT_TYPE_MAX) {
		LOG_INF("type id (%d) is not valid\n", type_id);
		return -1;
	}
...
int flashlight_get_ct_index(int ct_id)
{
	if (ct_id < 1 || ct_id > FLASHLIGHT_CT_MAX) {
		LOG_INF("color temperature id (%d) is not valid\n", ct_id);
		return -1;
	}
...
int flashlight_get_part_index(int part_id)
{
	if (part_id < 1 || part_id > FLASHLIGHT_PART_MAX) {
		LOG_INF("part id (%d) is not valid\n", part_id);
		return -1;
	}
	...
	for (i = 0; i < flashlight_device_num; i++) {
		if (!strncmp(name, flashlight_id[i].name, FLASHLIGHT_NAME_SIZE)) {
			...
			if (flashlight_verify_index(type_index, ct_index, part_index)) {
				LOG_INF("Failed to register device (%s)\n", flashlight_id[i].name);
				continue;
			}
...
int flashlight_dev_unregister(const char *name)
{
	...
	for (i = 0; i < flashlight_device_num; i++) {
		if (!strncmp(name, flashlight_id[i].name, FLASHLIGHT_NAME_SIZE)) {
			...
			if (flashlight_verify_index(type_index, ct_index, part_index)) {
				LOG_INF("Failed to unregister device (%s)\n", flashlight_id[i].name);
				continue;
			}
		...
		struct flashlight_operations *dev_ops)
{
	...
	if (flashlight_verify_index(dev_id->type, dev_id->ct, dev_id->part)) {
		LOG_INF("Failed to register device (%d,%d,%d)\n",
				dev_id->type, dev_id->ct, dev_id->part);
		return -EINVAL;
	}

	LOG_INF("Register device (%d,%d,%d)\n",
			dev_id->type, dev_id->ct, dev_id->part);
...
int flashlight_dev_unregister_by_device_id(struct flashlight_device_id *dev_id)
{
	...
	if (flashlight_verify_index(dev_id->type, dev_id->ct, dev_id->part)) {
		LOG_INF("Failed to unregister device (%d,%d,%d)\n",
				dev_id->type, dev_id->ct, dev_id->part);
		return -EINVAL;
	}

	LOG_INF("Unregister device (%d,%d,%d)\n",
			dev_id->type, dev_id->ct, dev_id->part);
...
static int flashlight_update_charger_status(struct flashlight_dev *fdev)
{
	...
	if (fdev->ops->flashlight_ioctl(
		...
		LOG_INF("Failed to get charger status\n");
	else
...
static long _flashlight_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (copy_from_user(&fl_arg, (void __user *)arg, sizeof(struct flashlight_user_arg))) {
		LOG_INF("Failed copy arguments from user\n");
		return -EFAULT;
	}

//	LOG_INF("fl_arg.type_id=%d   fl_arg.ct_id=%d\n", fl_arg.type_id, fl_arg.ct_id );
	/* find flashlight device */
	mutex_lock(&fl_mutex);
	fdev = flashlight_find_dev_by_index(
			flashlight_get_type_index(fl_arg.type_id),// 1
			flashlight_get_ct_index(fl_arg.ct_id));// 1 
	mutex_unlock(&fl_mutex);
	if (!fdev) {
		LOG_INF("Find no flashlight device\n");
		return -EINVAL;
	}
	...
	if (flashlight_verify_index(type, ct, part)) {
		LOG_INF("Failed with error index\n");
		return -EINVAL;
	}

	switch (cmd) {
	case FLASH_IOC_GET_PROTOCOL_VERSION:
		LOG_INF("case FLASH_IOC_GET_PROTOCOL_VERSION(%d,%d,%d): %d\n",
				type, ct, part, FLASHLIGHT_PROTOCOL_VERSION);
		ret = FLASHLIGHT_PROTOCOL_VERSION;
		break;

	case FLASH_IOC_IS_LOW_POWER:
		LOG_INF("case FLASH_IOC_IS_LOW_POWER(%d,%d,%d)\n",type, ct, part);
		fl_arg.arg = 0;
#ifdef CONFIG_MTK_FLASHLIGHT_PT
		fl_arg.arg = pt_is_low(pt_low_vol, pt_low_bat, pt_over_cur);
		if (fl_arg.arg)
			LOG_INF("Pt status: (%d,%d,%d)\n", pt_low_vol, pt_low_bat, pt_over_cur);
#endif
		if (copy_to_user((void __user *)arg, (void *)&fl_arg, sizeof(struct flashlight_user_arg))) 
        {
			LOG_INF("Failed to copy power status to user\n");
			return -EFAULT;
		}
		break;

	case FLASH_IOC_LOW_POWER_DETECT_START:
		LOG_INF("case FLASH_IOC_LOW_POWER_DETECT_START(%d,%d,%d): %d\n",type, ct, part, fl_arg.arg);
		mutex_lock(&fl_mutex);
	...
	case FLASH_IOC_LOW_POWER_DETECT_END:
		LOG_INF("case FLASH_IOC_LOW_POWER_DETECT_END(%d,%d,%d)\n", type, ct, part);
		mutex_lock(&fl_mutex);
		fdev->low_pt_level = -1;
		mutex_unlock(&fl_mutex);
		break;
	...
	case FLASH_IOC_IS_CHARGER_READY:
		...
		LOG_INF("case FLASH_IOC_IS_CHARGER_READY(%d,%d,%d): %d\n",type, ct, part, fl_arg.arg);
		if (copy_to_user((void __user *)arg, (void *)&fl_arg, sizeof(struct flashlight_user_arg))) 
        {
			LOG_INF("Failed to copy charger status to user\n");
			return -EFAULT;
		}
		break;
	...
	case FLASH_IOC_IS_HARDWARE_READY:
		if (fdev->ops)
			...
			fl_arg.arg = 0;
		LOG_INF("case FLASH_IOC_IS_HARDWARE_READY(%d,%d,%d): %d\n",type, ct, part, fl_arg.arg);
		if (copy_to_user((void __user *)arg, (void *)&fl_arg,sizeof(struct flashlight_user_arg))) 
        {
			LOG_INF("Failed to copy hardware status to user\n");
			return -EFAULT;
		}
		break;

	case FLASHLIGHTIOC_X_SET_DRIVER:
		LOG_INF("case FLASHLIGHTIOC_X_SET_DRIVER(%d,%d,%d): %d\n",type, ct, part, fl_arg.arg);
		if (fdev->ops)
	...
	case FLASH_IOC_SET_SCENARIO:
		LOG_INF("case FLASH_IOC_SET_SCENARIO(%d,%d,%d): %d\n",type, ct, part, fl_arg.arg);
		if (fdev->ops) 
        {
	...
	case FLASH_IOC_GET_MAIN2_PART_ID:
		...
		LOG_INF("case FLASH_IOC_GET_PART_ID(%d,%d,%d): %d\n",type, ct, part, fl_arg.arg);
		if (copy_to_user((void __user *)arg, (void *)&fl_arg,sizeof(struct flashlight_user_arg))) 
        {
			LOG_INF("Failed to copy part id to user\n");
			return -EFAULT;
		}
		break;

	case FLASH_IOC_SET_DUTY:
		LOG_INF("case FLASH_IOC_SET_DUTY(%d,%d,%d): %d\n",type, ct, part, fl_arg.arg);
		mutex_lock(&fl_mutex);
	...
	case FLASH_IOC_SET_ONOFF:
		LOG_INF("case FLASH_IOC_SET_ONOFF(%d,%d,%d): %d\n",type, ct, part, fl_arg.arg);
		mutex_lock(&fl_mutex);
		...
		const char *buf, size_t size)
{
	...
	LOG_INF("Strobe store\n");

	while (cur) {
		token = strsep(&cur, delim);
		ret = kstrtos32(token, 10, &num);
		if (ret) {
			LOG_INF("Error arguments\n");
			goto unlock;
		}
	...
	if (count != FLASHLIGHT_ARG_NUM) {
		LOG_INF("Error argument number: (%d)\n", count);
		ret = -1;
		goto unlock;
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	LOG_INF("Power throttling store\n");
	...
	LOG_INF("PT status (%d, %d, %d) with strict(%d)\n",
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (charger_status_tmp < FLASHLIGHT_CHARGER_NOT_READY ||
		...
		LOG_INF("Error arguments charger status(%d)\n", charger_status_tmp);
		ret = -1;
		goto unlock;
	}
...
static int flashlight_probe(struct platform_device *dev)
{
	...
	if (alloc_chrdev_region(&flashlight_devno, 0, 1, FLASHLIGHT_DEVNAME)) {
		LOG_INF("Failed to allocate char device region\n");
		goto err_allocate_chrdev;
	}
	LOG_INF("Allocate major number and minor number: (%d, %d)\n",
	...
	if (!flashlight_cdev) {
		LOG_INF("Failed to allcoate cdev\n");
	...
	if (IS_ERR(flashlight_class)) {
		LOG_INF("Failed to create class (%d)\n", (int)PTR_ERR(flashlight_class));
	...
	if (!flashlight_device) {
		LOG_INF("Failed to create device\n");
...
//brightness_level( 0=off  1~7=torch  >7=will do nothing ) 
int ontim_torch_onoff(int brightness_level)
{
    int ret=0;
 	struct flashlight_dev *fdev;
    struct flashlight_dev_arg fl_dev_arg;
    ...
    mutex_lock(&fl_mutex);
    fdev = flashlight_find_dev_by_index(flashlight_get_type_index(1), flashlight_get_ct_index(1));
    mutex_unlock(&fl_mutex);
    ...
    if(brightness_level == 0)
    {
        ...
        if (fdev->ops)
        {
            ret = fdev->ops->flashlight_set_driver(0);//  uninit device
        }
        else 
        {
            LOG_INF("Failed with no flashlight ops\n");
    ...
    else if(brightness_level <= 7) 
    {
        if (fdev->ops)
        {
            ret = fdev->ops->flashlight_set_driver(1);//  init device
        }
        else 
        {
            LOG_INF("Failed with no flashlight ops\n");
		...
		LOG_INF(" torch on   brightness_level=%d \n", brightness_level);
		mutex_lock(&fl_mutex);
		ret = fl_set_level(fdev, brightness_level - 1);
		mutex_unlock(&fl_mutex);
		...
		if (fdev->ops)
        {
			ret = fdev->ops->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, (unsigned long)&fl_dev_arg);
        }
		else 
        {
			LOG_INF("Failed with no flashlight ops\n");
