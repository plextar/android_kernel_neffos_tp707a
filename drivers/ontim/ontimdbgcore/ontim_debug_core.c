			 ...
			 const char *buf, size_t count)
{
    ...
    if(ontim_debug){
        ...
        for(i=0;i<ontim_debug->dev_attr_array_len;i++)
        {
			...
			if(strnstr(buf,"0x",2))
			    res=kstrtouint(buf, 16, (unsigned int *)&val);
			else
			    res=kstrtouint(buf, 10, (unsigned int *)&val);
        ...
        const char *buf, size_t count)
{
    ...
    struct ontim_debug *ontim_debug;
    mutex_lock(&sysfs_lock);
    res = kstrtouint(buf, 10, (unsigned int *)&dbg_trace_level);
...
int ontim_dev_debug_file_exist(struct ontim_debug *ontim_debug)
{
    ...
    ontim_debug_sd = (struct sysfs_dirent	*)sysfs_get_dirent(ontim_core_dbg_kobj->sd, ( unsigned char *)ontim_debug->dev_name);
    if (!ontim_debug_sd) {
        retval = -ENODEV;
...
//    ontim_debug->dbg_trace_level=0;
    ontim_debug_sd = (struct sysfs_dirent	*)sysfs_get_dirent(ontim_core_dbg_kobj->sd, ( unsigned char *)ontim_core_dbg_item->name);
    if (ontim_debug_sd) {
	retval = -ENODEV;
...
static unsigned int gpio_base =343;
...
#else
      if(gpio_is_valid(gpio_base+gpio_force_num)&&(gpio_force_num!=0))
      {
          status=!!gpio_get_value(gpio_base+gpio_force_num);
			  ...
			  const char *buf, size_t count)
{
    ...
    mutex_lock(&sysfs_lock);
    if(strnstr(buf,"0x",2))
        res = kstrtouint(buf, 16, (unsigned int *)&gpio_force_num);
    else
	res = kstrtouint(buf, 10, (unsigned int *)&gpio_force_num);
#ifdef CONFIG_MTK_GPIO
			  ...
			  const char *buf, size_t count)
{
    ...
    mutex_lock(&sysfs_lock);
    if(strnstr(buf,"0x",2))
        res = kstrtouint(buf, 16, (unsigned int *)&gpio_value);
    else
	res = kstrtouint(buf, 10, (unsigned int *)&gpio_value);
#ifdef CONFIG_MTK_GPIO
 	mt_set_gpio_mode(gpio_force_num, GPIO_MODE_00);
    ...
    if(gpio_is_valid(gpio_base+gpio_force_num)&&(gpio_force_num!=0))
    {
        res=gpio_direction_output(gpio_base+gpio_force_num,!!gpio_value);
